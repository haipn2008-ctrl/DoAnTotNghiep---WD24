...
class DemoPropertySeeder extends Seeder
{
    ...
    private function seedRooms(): array
    {
        $definitions = [
            ...
            ['B201', 2, 3800000, 30, 3, Room::STATUS_OCCUPIED, ['Wifi', 'Máy lạnh', 'Ban công', 'Tủ lạnh'], 'Phòng rộng có ban công hướng đông, đón nắng sáng và nhiều ánh sáng tự nhiên.'],
            ...
            ['B204', 2, 3700000, 29, 2, Room::STATUS_OCCUPIED, ['Wifi', 'Máy lạnh', 'Ban công'], 'Phòng cuối hành lang, ít tiếng ồn, có ban công nhỏ trồng cây.'],
            ['C301', 3, 4200000, 34, 3, Room::STATUS_OCCUPIED, ['Wifi', 'Máy lạnh', 'Tủ lạnh', 'Ban công'], 'Căn phòng lớn nhất tầng ba, phù hợp gia đình trẻ hoặc nhóm hai người.'],
            ['C302', 3, 3900000, 30, 2, Room::STATUS_AVAILABLE, ['Wifi', 'Máy lạnh', 'Thang máy'], 'Phòng gần thang máy, nội thất cơ bản đầy đủ và có thể vào ở ngay.'],
            ['C303', 3, 4000000, 32, 2, Room::STATUS_AVAILABLE, ['Wifi', 'Máy lạnh', 'Ban công'], 'Phòng hai mặt thoáng, ban công nhìn ra ngõ, khu vực sinh hoạt rộng.'],
            ['C304', 3, 3800000, 29, 2, Room::STATUS_AVAILABLE, ['Wifi', 'Nóng lạnh', 'Thang máy'], 'Phòng gọn gàng, phù hợp nhân viên văn phòng cần không gian yên tĩnh.'],
        ];
        ...
        foreach ($definitions as [$code, $floor, $price, $area, $people, $status, $amenities, $description]) {
            ...
            $room->amenities()->sync(Amenity::whereIn('name', $amenities)->pluck('id'));
            $rooms[$code] = $room;
        }
    ...
    private function seedContracts(array $rooms, array $tenants): array
    {
        ...
        $definitions = [
            ['HD-AP-2025-001', 'A101', 'giahuy@example.test', -10, 14, 2, Contract::STATUS_ACTIVE, 'paid'],
            ['HD-AP-2025-002', 'A102', 'ngocmai@example.test', -8, 16, 1, Contract::STATUS_ACTIVE, 'paid'],
            ['HD-AP-2026-003', 'A104', 'ducanh@example.test', -6, 12, 2, Contract::STATUS_ACTIVE, 'paid'],
            ['HD-AP-2026-004', 'B201', 'khanhlinh@example.test', -5, 13, 2, Contract::STATUS_ACTIVE, 'paid'],
            ['HD-AP-2026-005', 'B202', 'quangnam@example.test', -4, 12, 1, Contract::STATUS_ACTIVE, 'paid'],
            ['HD-AP-2026-006', 'B204', 'thanhthao@example.test', -3, 12, 2, Contract::STATUS_ACTIVE, 'paid'],
            ['HD-AP-2025-007', 'C301', 'tuankiet@example.test', -15, -2, 3, Contract::STATUS_EXPIRED, 'returned'],
            ['HD-AP-2025-008', 'C302', 'phuonguyen@example.test', -12, -4, 1, Contract::STATUS_TERMINATED, 'returned'],
        ];

        $contracts = [];
        foreach ($definitions as [$code, $roomCode, $email, $startOffset, $endOffset, $people, $status, $depositStatus]) {
            $startDate = $start->copy()->addMonths($startOffset);
            $endDate = $start->copy()->addMonths($endOffset)->endOfMonth();
            $room = $rooms[$roomCode];
            $tenant = $tenants[$email];
            $ended = in_array($status, [Contract::STATUS_EXPIRED, Contract::STATUS_TERMINATED], true);

                    ...
                    <tr class="hover:bg-slate-50/70">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex justify-end gap-2">
                                ...
                                @endif
                                @if ($tenant->status === \App\Models\Tenant::STATUS_ARCHIVED)
                                    <form action="{{ route('admin.tenants.restore', $tenant) }}" method="POST" data-confirm="Hồ sơ khách thuê và tài khoản liên kết sẽ được khôi phục." data-confirm-label="Khôi phục hồ sơ" data-reason-input="restoration_reason" data-reason-placeholder="Nhập lý do khôi phục khách thuê...">@csrf @method('PATCH')<input type="hidden" name="restoration_reason"><button type="submit" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100" title="Khôi phục"><i class="bx bx-revision text-lg"></i></button></form>
                                @elseif (! $activeContract)
                                    <form action="{{ route('admin.tenants.destroy', $tenant) }}" method="POST" data-confirm="Hồ sơ sẽ được lưu trữ, dữ liệu và giấy tờ vẫn được giữ lại." data-confirm-label="Lưu trữ hồ sơ" data-reason-input="archive_reason" data-reason-placeholder="Nhập lý do lưu trữ khách thuê...">@csrf @method('DELETE')<input type="hidden" name="archive_reason"><button type="submit" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100" title="Lưu trữ"><i class="bx bx-archive text-lg"></i></button></form>
                                @endif
                            </div>
                        </td>
                    </tr>

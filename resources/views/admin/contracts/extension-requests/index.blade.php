        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-slate-900">{{ $extensionRequests->count() }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-amber-200 bg-amber-50/60 p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-amber-700">{{ $extensionRequests->where('status', 'pending')->count() }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-emerald-700">{{ $extensionRequests->where('status', 'approved')->count() }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-rose-200 bg-rose-50/60 p-5 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-3xl font-bold text-rose-700">{{ $extensionRequests->where('status', 'rejected')->count() }}</p>
                </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Hạn hiện tại</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Đề nghị đến</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Lý do</th>
                        ...
                        <tr class="transition hover:bg-slate-50/80">
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-50 font-bold text-indigo-600">HĐ</div>
                                    <div>
                                        <div class="font-semibold text-slate-900">{{ $request->contract->contract_code ?? '-' }}</div>
                                        <div class="mt-0.5 text-xs text-slate-500">Phòng {{ $request->contract->room->room_code ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>

                            <td class="px-5 py-4">
                                <div class="font-medium text-slate-900">{{ $request->contract->tenant->full_name ?? '-' }}</div>
                                <div class="mt-1 text-xs text-slate-500">{{ $request->contract->tenant->phone ?? 'Chưa có SĐT' }}</div>
                            </td>

                            <td class="whitespace-nowrap px-5 py-4 text-sm font-medium text-slate-700">
                                {{ optional($request->current_end_date)->format('d/m/Y') ?? '-' }}
                            </td>

                            <td class="whitespace-nowrap px-5 py-4 text-sm font-bold text-indigo-600">
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                @if($request->status === 'pending')
                                    <span class="inline-flex items-center rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700">Chờ duyệt</span>
                                @elseif($request->status === 'approved')
                                    <span class="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700">Đã duyệt</span>
                                @elseif($request->status === 'rejected')
                                    <span class="inline-flex items-center rounded-full border border-rose-200 bg-rose-50 px-2.5 py-1 text-xs font-semibold text-rose-700">Từ chối</span>
                                @endif
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-4 text-center">
                                @if($request->status === 'pending')
                                    <div class="flex items-center justify-center gap-2">
                                        <form method="POST"
                                              action="{{ route('admin.extension-requests.approve', $request) }}"
                                              onsubmit="return confirm('Bạn chắc chắn muốn duyệt yêu cầu gia hạn này?')">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form method="POST"
                                              action="{{ route('admin.extension-requests.reject', $request) }}"
                                              onsubmit="return confirm('Bạn chắc chắn muốn từ chối yêu cầu gia hạn này?')">
                                            @csrf
                                            <button type="submit"
                                ...
                                @else
                                    <span class="text-sm font-medium text-slate-400">Đã xử lý</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                ...
                                <h3 class="mt-4 font-semibold text-slate-900">Chưa có yêu cầu gia hạn</h3>
                                <p class="mt-1 text-sm text-slate-500">Yêu cầu của khách thuê sẽ xuất hiện tại đây.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>

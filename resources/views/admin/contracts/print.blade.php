...
@php
    ...
    <table class="meta"><tr><td class="label">Thời hạn thuê</td><td>{{ $contract->start_date->format('d/m/Y') }} đến {{ $contract->end_date->format('d/m/Y') }}</td></tr><tr><td class="label">Tiền thuê</td><td>{{ number_format($contract->monthly_rent,0,',','.') }}đ/tháng</td></tr><tr><td class="label">Tiền phòng tháng đầu trả trước</td><td>{{ number_format($contract->deposit_amount,0,',','.') }}đ — cấn vào tháng đầu, không hoàn cuối hợp đồng</td></tr><tr><td class="label">Số người dự kiến</td><td>{{ $contract->number_of_people }}</td></tr><tr><td class="label">Ngày nhận phòng dự kiến</td><td>{{ $contract->scheduled_move_in_date?->format('d/m/Y') ?: '—' }}</td></tr></table>

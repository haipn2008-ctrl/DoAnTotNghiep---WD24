...
@php
    ...
    <table class="meta"><tr><td class="label">Internet</td><td>{{ $contract->internet_enabled ? 'Có đăng ký' : 'Không đăng ký' }}</td></tr><tr><td class="label">Dịch vụ chung</td><td>{{ $contract->service_enabled ? 'Có đăng ký' : 'Không đăng ký' }}</td></tr><tr><td class="label">Giữ xe</td><td>{{ $contract->parking_quantity > 0 ? $contract->parking_quantity.' xe' : 'Không đăng ký' }}</td></tr></table>

...
<div class="mx-auto max-w-5xl space-y-5">
    ...
    <form method="POST" action="{{ route('admin.contracts.update', $contract) }}" enctype="multipart/form-data" data-contract-schedule data-ajax-validation-form novalidate class="rounded-xl border bg-white shadow-sm">
        ...
        <div class="grid gap-5 p-6 md:grid-cols-2">
            ...
            <div class="rounded-lg border border-indigo-200 bg-indigo-50 p-4 md:col-span-2"><p class="text-sm font-semibold text-indigo-950">Cọc {{ number_format($contract->deposit_amount, 0, ',', '.') }}đ + phòng tháng đầu {{ number_format($contract->first_month_rent_amount, 0, ',', '.') }}đ</p><p class="mt-1 text-sm text-indigo-800">Tổng ban đầu {{ number_format($contract->deposit_amount + $contract->first_month_rent_amount, 0, ',', '.') }}đ. Tháng đầu có {{ $contract->first_month_rent_days }} ngày tính từ ngày bắt đầu đến cuối tháng{{ $contract->first_month_rent_days <= 5 ? ' và được miễn tiền phòng' : '' }}.</p></div>

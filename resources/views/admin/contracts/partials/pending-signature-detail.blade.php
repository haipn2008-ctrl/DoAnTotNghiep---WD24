...
<section class="overflow-hidden rounded-lg border border-emerald-200 bg-white shadow-sm">
    <div class="border-b border-emerald-100 bg-emerald-50 px-4 py-3">
        <h3 class="font-semibold text-emerald-950">Xác nhận ký</h3>
    </div>
    <form class="lifecycle-form grid grid-cols-1 gap-4 p-4 xl:grid-cols-12 xl:items-end" method="POST" enctype="multipart/form-data" action="{{ route('admin.contracts.mark-signed', $contract) }}">
        @csrf
        ...
        <button class="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 text-sm font-semibold text-white hover:bg-emerald-700 xl:col-span-2">
...
<div class="space-y-4">
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 font-semibold text-slate-800">
            <span>Thông tin hợp đồng và người thuê</span>
        ...
        <div class="space-y-3 p-4">
            <dl class="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-lg bg-slate-50 px-3 py-2.5"><dt class="text-xs font-medium text-slate-500">Ngày dự kiến nhận</dt><dd class="mt-1 font-semibold leading-5 text-slate-900">{{ $contract->scheduled_move_in_date?->format('d/m/Y') ?? '—' }}</dd></div>
                <div class="rounded-lg bg-slate-50 px-3 py-2.5"><dt class="text-xs font-medium text-slate-500">Hạn giữ phòng</dt><dd class="mt-1 font-semibold leading-5 text-slate-900">{{ $contract->reservation_expires_at?->format('d/m/Y H:i') ?? '—' }}</dd></div>
                <div class="rounded-lg bg-slate-50 px-3 py-2.5"><dt class="text-xs font-medium text-slate-500">Tiền phòng</dt><dd class="mt-1 font-semibold leading-5 text-slate-900">{{ number_format($contract->monthly_rent, 0, ',', '.') }}đ/tháng</dd></div>
                <div class="rounded-lg bg-slate-50 px-3 py-2.5"><dt class="text-xs font-medium text-slate-500">Hạn ký</dt><dd class="mt-1 font-semibold leading-5 text-slate-900">{{ $contract->signature_due_at?->format('d/m/Y H:i') ?? '—' }}</dd></div>
            </dl>
            <div class="grid gap-2 md:grid-cols-2">
                @foreach($contract->currentMembers as $member)
                    <article class="min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-3 text-sm">
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-3 font-semibold text-slate-800">
            <span>Bàn giao và chi phí định kỳ</span>
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 px-4 py-3 font-semibold text-slate-800">Lịch sử hợp đồng</div>
        <div class="p-4">

...
<section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3 sm:px-5">
        <h3 class="font-bold text-slate-950">Tổng quan hợp đồng</h3>
        <span class="text-xs font-medium text-slate-500">Cập nhật đến {{ now()->format('d/m/Y') }}</span>
    </div>
    <div class="grid divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-4">
        <div class="px-4 py-3.5">
            ...
            <p class="mt-1 text-lg font-bold text-slate-950">{{ number_format((float) $contract->monthly_rent, 0, ',', '.') }}đ</p>
        </div>
        <div class="px-4 py-3.5">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Còn phải thu</p>
            <p class="mt-1 text-lg font-bold {{ $totalOutstanding > 0 ? 'text-rose-700' : 'text-emerald-700' }}">{{ number_format($totalOutstanding, 0, ',', '.') }}đ</p>
        </div>
        <div class="px-4 py-3.5">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Thời hạn còn lại</p>
            <p class="mt-1 text-lg font-bold {{ $daysRemaining <= 30 ? 'text-amber-700' : 'text-slate-950' }}">{{ max(0, $daysRemaining) }} ngày</p>
        </div>
        <div class="px-4 py-3.5">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">Tiền cọc đang giữ</p>
            <p class="mt-1 text-lg font-bold text-slate-950">{{ number_format((float) $contract->deposit_amount, 0, ',', '.') }}đ</p>
        </div>
    </div>
</section>
...
<div id="contract-overview" class="scroll-mt-24 grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,.55fr)]">
    <div class="space-y-4">
                ...
                <div class="border-b border-slate-100 p-4">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div class="flex min-w-0 gap-3">
                            ...
                            <div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><p class="font-bold text-slate-950">{{ $representative->full_name }}</p><span class="rounded-full bg-indigo-100 px-2 py-0.5 text-[11px] font-semibold text-indigo-700">Người đại diện</span></div><p class="mt-1 text-sm text-slate-600">{{ $representative->phone ?: 'Chưa có số điện thoại' }} · {{ $representative->tenant?->user?->email ?: 'Chưa có email' }} · CCCD {{ $representative->identity_number ?: 'chưa cập nhật' }}</p></div>
                        </div>
                        ...
                        <article class="px-5 py-4">
                            ...
                            @if($canRestoreMoveOut)
                                <details class="mt-3 rounded-xl border border-amber-200 bg-amber-50/60 p-3">
                                    ...
                                    <form method="POST" action="{{ route('admin.contract-tenants.restore-move-out', $member) }}" class="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]" data-confirm="Người thuê sẽ được khôi phục vào danh sách đang ở." data-confirm-label="Khôi phục người thuê">
                                        @csrf
        ...
        <section id="contract-assets" class="scroll-mt-24 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="border-t border-slate-200">
                <div class="grid gap-3 p-5 sm:grid-cols-2"><div class="rounded-xl bg-indigo-50 p-4"><p class="font-semibold text-indigo-950">Internet</p><p class="mt-1 text-sm text-indigo-700">{{ number_format((float) $setting->internet_fee, 0, ',', '.') }}đ/người/tháng</p></div><div class="rounded-xl bg-emerald-50 p-4"><p class="font-semibold text-emerald-950">Dịch vụ chung</p><p class="mt-1 text-sm text-emerald-700">{{ number_format((float) $setting->service_fee, 0, ',', '.') }}đ/người/tháng</p></div></div>
    ...
    <aside class="space-y-4 xl:sticky xl:top-5 xl:self-start">
        <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="p-5">
                <div class="flex items-center justify-between"><h3 class="font-bold text-slate-950">Thu tiền hợp đồng</h3><span class="rounded-full px-2.5 py-1 text-xs font-bold {{ $totalOutstanding > 0 ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700' }}">Đã thu {{ $collectionProgress }}%</span></div>
                <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100"><div class="h-full rounded-full {{ $totalOutstanding > 0 ? 'bg-indigo-600' : 'bg-emerald-500' }}" style="width: {{ $collectionProgress }}%"></div></div>
                <dl class="mt-3 divide-y divide-slate-100 text-sm"><div class="flex justify-between gap-3 py-2"><dt class="text-slate-500">Đã xuất hóa đơn</dt><dd class="font-semibold">{{ number_format($totalInvoiced, 0, ',', '.') }}đ</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-slate-500">Đã thanh toán</dt><dd class="font-semibold text-emerald-700">{{ number_format($totalPaid, 0, ',', '.') }}đ</dd></div><div class="flex justify-between gap-3 py-2"><dt class="text-slate-500">Còn phải thu</dt><dd class="font-bold {{ $totalOutstanding > 0 ? 'text-rose-700' : 'text-emerald-700' }}">{{ number_format($totalOutstanding, 0, ',', '.') }}đ</dd></div></dl>
                <a href="{{ route('admin.invoices.index', ['keyword' => $contract->contract_code]) }}" class="mt-3 flex h-9 items-center justify-center rounded-lg border border-indigo-200 bg-indigo-50 text-sm font-semibold text-indigo-700 hover:bg-indigo-100">Xem tất cả hóa đơn</a>
            </div>
            <div class="border-t border-slate-200 bg-slate-50/60 p-5">
                <div class="flex items-center justify-between"><h3 class="font-bold text-slate-950">Thời hạn</h3><span class="text-xs font-semibold text-slate-500">{{ $termProgress }}% đã qua</span></div>
                <div class="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200"><div class="h-full rounded-full bg-emerald-500" style="width: {{ $termProgress }}%"></div></div>
                <div class="mt-2 flex justify-between text-xs text-slate-500"><span>{{ $contract->start_date->format('d/m/Y') }}</span><span>{{ $contract->end_date->format('d/m/Y') }}</span></div>
                <dl class="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><dt class="text-xs text-slate-500">Nhận phòng</dt><dd class="mt-0.5 font-semibold">{{ $contract->actual_move_in_at?->format('d/m/Y H:i') ?: '—' }}</dd></div><div><dt class="text-xs text-slate-500">Số người</dt><dd class="mt-0.5 font-semibold">{{ $contract->currentMembers->count() }}/{{ $contract->room?->max_people }}</dd></div></dl>
            </div>
        </section>
    </aside>
</div>

    ...
    <div class="grid gap-6 lg:grid-cols-2">
        <section class="rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <div class="grid gap-4 p-5 sm:grid-cols-2">
                ...
                <div class="rounded-lg bg-slate-50 p-4"><p class="text-sm text-slate-500">Cọc + tiền phòng tháng đầu</p><p class="mt-2 text-lg font-bold text-slate-950">{{ number_format($contract->deposit_amount + $contract->first_month_rent_amount, 0, ',', '.') }}đ</p><p class="mt-1 text-xs text-slate-500">{{ $contract->first_month_rent_days <= 5 ? 'Tháng đầu được miễn tiền phòng' : 'Tháng đầu tính '.$contract->first_month_rent_days.' ngày' }}</p></div>
            </div>
        </section>

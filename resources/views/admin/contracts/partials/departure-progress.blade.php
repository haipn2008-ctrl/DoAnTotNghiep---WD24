        ...
        @foreach($departureSteps as $step)
            <article class="rounded-xl border p-4 {{ $step['done'] ? 'border-emerald-200 bg-emerald-50/60' : ($step['active'] ? 'border-violet-300 bg-violet-50 ring-2 ring-violet-100' : 'border-slate-200 bg-slate-50 opacity-65') }}">
                <div class="flex items-start gap-3">
                    ...
                    <div><p class="text-xs font-bold uppercase tracking-wide {{ $step['done'] ? 'text-emerald-700' : ($step['active'] ? 'text-violet-700' : 'text-slate-500') }}">{{ $step['done'] ? 'Đã xong' : ($step['active'] ? 'Đang thực hiện' : 'Đang khóa') }}</p><h4 class="mt-1 font-bold text-slate-950">{{ $step['title'] }}</h4><p class="mt-1 text-xs leading-5 text-slate-500">{{ $step['description'] }}</p>@if(($showCheckoutLink ?? false) && $step['active'])<a href="{{ route('admin.contracts.check-out.form', $contract) }}" class="mt-3 inline-flex h-9 items-center gap-1.5 rounded-lg border border-indigo-300 bg-indigo-50 px-3 text-xs font-bold text-indigo-700 shadow-sm hover:bg-indigo-100 hover:text-indigo-800">Mở bước xử lý<i class="bx bx-right-arrow-alt text-base"></i></a>@endif</div>
                </div>
            </article>
        @endforeach
    </div>

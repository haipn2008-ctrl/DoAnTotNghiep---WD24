...
@php
    $statusData = [
        ...
        'pending_deposit' => ['label' => 'Chờ đóng cọc', 'class' => 'bg-orange-50 text-orange-700 ring-orange-200', 'dot' => 'bg-orange-500'],
    ...
    $actionLabels = [
        ...
        'deposit_completed' => 'Xác nhận đã thu đủ cọc',
    ...
    $depositResolutionLabels = ['refunded' => 'Đã hoàn cọc', 'deducted' => 'Đã khấu trừ', 'retained' => 'Đã giữ lại'];
    $conditionLabels = [
...
<div class="space-y-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><p class="text-sm font-medium text-slate-500">Tiền cọc phải thu</p><span class="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-50 text-amber-600"><i class="bx bx-wallet text-xl"></i></span></div><p class="mt-4 text-xl font-bold text-slate-950">{{ number_format($contract->deposit_amount,0,',','.') }}đ</p></div>
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><p class="text-sm font-medium text-slate-500">Cọc còn thiếu</p><span class="flex h-10 w-10 items-center justify-center rounded-lg bg-rose-50 text-rose-600"><i class="bx bx-error-circle text-xl"></i></span></div><p class="mt-4 text-xl font-bold text-rose-700">{{ number_format($depositRemaining,0,',','.') }}đ</p></div>
    </div>
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div class="grid gap-4 p-5 lg:grid-cols-2">
            ...
            @if($contract->status===\App\Models\Contract::STATUS_PENDING_SIGNATURE)
                ...
                <div class="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600 lg:col-span-2"><strong class="text-slate-800">Chưa thể thu cọc hoặc nhận phòng:</strong> quản trị viên cần xác nhận thời điểm ký hợp lệ trước.</div>
            @endif
            ...
            @if($contract->status===\App\Models\Contract::STATUS_SETTLING)
                <div class="rounded-lg border border-dashed p-4 text-sm text-slate-600 lg:col-span-2"><strong>Điều kiện hoàn tất:</strong> không còn hóa đơn chưa thanh toán hoặc mới thanh toán một phần (trừ khoản được xóa nợ có lý do), đồng thời tiền cọc phải được hoàn, khấu trừ hoặc giữ lại có chứng từ.</div>

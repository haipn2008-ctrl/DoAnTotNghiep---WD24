...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div class="grid gap-4 p-5 lg:grid-cols-2">
            ...
            @endif
            @if($contract->status===\App\Models\Contract::STATUS_PENDING_SIGNATURE)
                <form class="lifecycle-form rounded-xl border border-emerald-200 bg-emerald-50/40 p-4" method="POST" enctype="multipart/form-data" action="{{ route('admin.contracts.mark-signed',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Xác nhận đã ký</h4><label class="mt-3 block text-sm font-semibold text-slate-700">Thời điểm ký</label><input type="datetime-local" name="signed_at" required class="mt-1.5 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"><label class="mt-3 block text-sm font-semibold text-slate-700">Bản hợp đồng đã ký (không bắt buộc)</label><input type="file" name="signed_contract_file" accept=".pdf,image/jpeg,image/png,image/webp" class="mt-1.5 block w-full rounded-lg border border-slate-200 bg-white text-sm file:mr-3 file:border-0 file:bg-slate-100 file:px-3 file:py-2.5"><textarea name="reason" rows="2" placeholder="Ghi chú (không bắt buộc)" class="mt-3 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"></textarea><button class="mt-3 inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700"><i class="bx bx-check-circle text-lg"></i>Xác nhận đã ký</button></form>
            ...
            @if($contract->status===\App\Models\Contract::STATUS_PENDING_DEPOSIT)
                <form class="lifecycle-form rounded-xl border border-orange-200 bg-orange-50/40 p-4 lg:col-span-2" method="POST" action="{{ route('admin.contracts.deposit-invoice.issue',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Tiền cọc</h4><button class="mt-3 rounded-lg bg-orange-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-orange-700">Phát hành hóa đơn</button></form>
            @endif
            @if($contract->status===\App\Models\Contract::STATUS_AWAITING_MOVE_IN)
                <form class="lifecycle-form rounded-xl border border-emerald-200 bg-emerald-50/40 p-4" method="POST" action="{{ route('admin.contracts.check-in',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Xác nhận nhận phòng</h4>@if(!$contract->move_in_details_confirmed_at)<p class="mt-2 rounded-lg bg-amber-50 p-2 text-sm font-medium text-amber-800">Khách chưa xác nhận thông tin.</p>@endif<div class="mt-3 grid gap-2 sm:grid-cols-2"><input type="datetime-local" name="actual_move_in_at" required class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"><input type="number" min="0" name="handover_electricity" value="{{ old('handover_electricity', $suggestedHandoverReading?->electricity_new) }}" required placeholder="Điện bàn giao" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"><input type="number" min="0" name="handover_water" value="{{ old('handover_water', $suggestedHandoverReading?->water_new) }}" required placeholder="Nước bàn giao" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"><input name="schedule_variance_reason" placeholder="Lý do nếu sớm/muộn" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"></div><label class="mt-3 flex gap-2 text-sm text-slate-700"><input type="checkbox" name="handover_confirmed" value="1" required class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"> Đã đối chiếu chỉ số và bàn giao</label><button @disabled(!$contract->move_in_details_confirmed_at) class="mt-3 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-300">Xác nhận nhận phòng</button></form>
                <form class="lifecycle-form rounded-xl border border-sky-200 bg-sky-50/40 p-4" method="POST" action="{{ route('admin.contracts.extend-move-in-deadline',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Gia hạn giữ phòng</h4><input type="datetime-local" name="reservation_expires_at" required class="mt-3 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-100"><textarea name="reason" rows="2" required placeholder="Lý do bắt buộc" class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-100"></textarea><button class="mt-3 rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-sky-700">Gia hạn giữ phòng</button></form>
            @endif
            @if(in_array($contract->status,\App\Models\Contract::OPEN_OCCUPANCY_STATUSES,true))
                <form class="lifecycle-form rounded-xl border border-violet-200 bg-violet-50/40 p-4" method="POST" action="{{ route('admin.contracts.check-out',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Xác nhận trả phòng</h4><div class="mt-3 grid gap-2 sm:grid-cols-2"><input type="datetime-local" name="actual_move_out_at" required class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100"><input type="number" min="0" name="checkout_electricity" value="{{ $latestReading?->electricity_new }}" required placeholder="Điện cuối" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100"><input type="number" min="0" name="checkout_water" value="{{ $latestReading?->water_new }}" required placeholder="Nước cuối" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100"><input type="number" min="0" name="settlement_amount" placeholder="Khoản quyết toán" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100"></div><textarea name="checkout_reason" rows="2" required placeholder="Lý do trả phòng" class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100"></textarea><input name="settlement_description" placeholder="Nội dung quyết toán" class="mt-2 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-violet-500 focus:ring-4 focus:ring-violet-100"><button class="mt-3 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-700">Xác nhận trả phòng</button></form>
                <form class="lifecycle-form rounded-xl border border-sky-200 bg-sky-50/40 p-4" method="POST" action="{{ route('admin.contracts.extend',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Gia hạn hợp đồng</h4><input type="date" name="new_end_date" required class="mt-3 h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-100"><textarea name="reason" rows="2" required placeholder="Lý do bắt buộc" class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-sky-500 focus:ring-4 focus:ring-sky-100"></textarea><button class="mt-3 rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-sky-700">Gia hạn</button></form>
            @endif
            @if($contract->status===\App\Models\Contract::STATUS_SETTLING)
                <form class="lifecycle-form rounded-xl border border-emerald-200 bg-emerald-50/40 p-4 lg:col-span-2" method="POST" action="{{ route('admin.contracts.complete-settlement',$contract) }}">@csrf<h4 class="font-semibold text-slate-950">Hoàn tất quyết toán</h4><div class="mt-3 grid gap-2 md:grid-cols-2"><select name="deposit_resolution" class="h-11 rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"><option value="">Chọn xử lý cọc</option><option value="refunded">Đã hoàn cọc</option><option value="deducted">Đã khấu trừ</option><option value="retained">Giữ cọc có chứng từ</option></select><textarea name="settlement_note" rows="2" placeholder="Chứng từ xử lý cọc" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"></textarea></div><label class="mt-3 flex gap-2 text-sm text-slate-700"><input type="checkbox" name="write_off_outstanding" value="1" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"> Xóa công nợ còn lại</label><textarea name="write_off_reason" rows="2" placeholder="Lý do xóa nợ" class="mt-2 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"></textarea><label class="mt-3 flex gap-2 text-sm text-slate-700"><input type="checkbox" name="confirm_complete" value="1" required class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500"> Xác nhận đã kiểm tra quyết toán</label><button class="mt-3 rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800">Hoàn tất hợp đồng</button></form>
            @endif

...
@php
    $statusData = [
        ...
        'pending_deposit' => ['label' => 'Chờ tiền phòng tháng đầu', 'class' => 'bg-orange-50 text-orange-700 ring-orange-200', 'dot' => 'bg-orange-500'],
    ...
    $actionLabels = [
        ...
        'deposit_completed' => 'Xác nhận đã thu đủ tiền phòng tháng đầu',
        'deposit_reversed' => 'Điều chỉnh lại tiền phòng tháng đầu',
...
<div class="space-y-6">
    ...
    @if($contract->isDepositOverdue())<div class="rounded-lg border border-orange-300 bg-orange-50 p-4 font-semibold text-orange-900">Quá hạn đóng tiền phòng tháng đầu.</div>@endif
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><p class="text-sm font-medium text-slate-500">Tiền phòng tháng đầu</p><span class="flex h-10 w-10 items-center justify-center rounded-lg bg-amber-50 text-amber-600"><i class="bx bx-wallet text-xl"></i></span></div><p class="mt-4 text-xl font-bold text-slate-950">{{ number_format($contract->deposit_amount,0,',','.') }}đ</p></div>
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><p class="text-sm font-medium text-slate-500">Đã thu</p><span class="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-50 text-emerald-600"><i class="bx bx-check-circle text-xl"></i></span></div><p class="mt-4 text-xl font-bold text-emerald-700">{{ number_format($depositPaid,0,',','.') }}đ</p></div>
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><p class="text-sm font-medium text-slate-500">Tháng đầu còn thiếu</p><span class="flex h-10 w-10 items-center justify-center rounded-lg bg-rose-50 text-rose-600"><i class="bx bx-error-circle text-xl"></i></span></div><p class="mt-4 text-xl font-bold text-rose-700">{{ number_format($depositRemaining,0,',','.') }}đ</p></div>
    </div>
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div class="grid gap-4 p-5 lg:grid-cols-2">
            ...
            @if($contract->status===\App\Models\Contract::STATUS_PENDING_SIGNATURE)
                ...
                <div class="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600 lg:col-span-2"><strong class="text-slate-800">Chưa thể thu tiền tháng đầu hoặc nhận phòng:</strong> quản trị viên cần xác nhận thời điểm ký hợp lệ trước.</div>
            @endif
            @if($contract->status===\App\Models\Contract::STATUS_PENDING_DEPOSIT)
                <form class="lifecycle-form rounded-lg border p-4" method="POST" action="{{ route('admin.contracts.deposit-invoice.issue',$contract) }}">@csrf<h4 class="font-semibold">Hóa đơn tiền phòng tháng đầu</h4><p class="mt-2 text-sm text-slate-500">Khoản thu bằng một tháng tiền phòng, được cấn vào tháng đầu và không hoàn lại cuối hợp đồng.</p><button class="mt-3 rounded bg-orange-600 px-4 py-2 text-white">Phát hành / mở hóa đơn tháng đầu</button></form>
                <div class="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600"><strong class="text-slate-800">Chưa thể nhận phòng:</strong> tổng số tiền thanh toán thành công phải đủ {{ number_format($contract->deposit_amount,0,',','.') }}đ. Các khoản đang chờ hoặc thất bại không được tính.</div>
            @endif

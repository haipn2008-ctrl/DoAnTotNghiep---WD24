    ...
    <section class="rounded-xl border bg-white shadow-sm">
        ...
        <div class="grid gap-4 p-5 lg:grid-cols-2">
            ...
            @if($contract->status===\App\Models\Contract::STATUS_AWAITING_MOVE_IN)
                <form class="lifecycle-form rounded-lg border p-4" method="POST" action="{{ route('admin.contracts.check-in',$contract) }}">@csrf<h4 class="font-semibold">Xác nhận nhận phòng</h4><div class="mt-3 grid gap-2 sm:grid-cols-2"><input type="datetime-local" name="actual_move_in_at" required class="h-10 rounded border px-2"><input type="number" min="0" name="handover_electricity" required placeholder="Điện bàn giao" class="h-10 rounded border px-2"><input type="number" min="0" name="handover_water" required placeholder="Nước bàn giao" class="h-10 rounded border px-2"><input name="schedule_variance_reason" placeholder="Lý do nếu sớm/muộn" class="h-10 rounded border px-2"></div><label class="mt-3 flex gap-2 text-sm"><input type="checkbox" name="handover_confirmed" value="1" required> Đã xác nhận biên bản bàn giao</label><button class="mt-3 rounded bg-emerald-600 px-4 py-2 text-white">Check-in</button></form>

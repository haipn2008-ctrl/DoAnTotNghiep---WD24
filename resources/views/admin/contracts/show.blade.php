...
<div class="space-y-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm"><p class="text-sm font-medium text-slate-500">Tiền phòng tháng đầu</p><p class="mt-4 text-xl font-bold text-slate-950">{{ number_format($contract->monthly_rent,0,',','.') }}đ</p><p class="mt-1 text-xs text-slate-500">Đã thu {{ number_format($firstMonthPaid,0,',','.') }}đ</p></div>
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        ...
        <div class="grid gap-4 p-5 lg:grid-cols-2">
            ...
            @if($contract->status===\App\Models\Contract::STATUS_PENDING_DEPOSIT)
                <form class="lifecycle-form rounded-lg border p-4" method="POST" action="{{ route('admin.contracts.deposit-invoice.issue',$contract) }}">@csrf<h4 class="font-semibold">Cọc và tiền phòng tháng đầu</h4><p class="mt-2 text-sm text-slate-500">Phát hành hai hóa đơn riêng, mỗi hóa đơn bằng một tháng tiền phòng.</p><button class="mt-3 rounded bg-orange-600 px-4 py-2 text-white">Phát hành / mở hai hóa đơn</button></form>
                <div class="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600"><strong class="text-slate-800">Chưa thể nhận phòng:</strong> phải thanh toán đủ cọc {{ number_format($contract->deposit_amount,0,',','.') }}đ và phòng tháng đầu {{ number_format($contract->monthly_rent,0,',','.') }}đ.</div>
            @endif

...
<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 px-5 py-4"><h3 class="font-semibold text-slate-950">Người thuê và danh sách cư trú</h3><p class="mt-1 text-sm text-slate-500">Người đại diện luôn là một người thuê trực tiếp. Hồ sơ được lưu độc lập và không bị xóa khỏi lịch sử.</p></div>
        <div class="grid gap-4 p-5 lg:grid-cols-2">
                ...
                <article class="rounded-lg border border-slate-200 p-4">
                    ...
                    @if($member->status === \App\Models\ContractTenant::STATUS_PENDING)
                        <div class="mt-3 grid gap-2 sm:grid-cols-2">
                            <form method="POST" action="{{ route('admin.contract-tenants.approve', $member) }}">@csrf<button class="w-full rounded bg-emerald-600 px-3 py-2 text-sm font-semibold text-white">Chấp nhận</button></form>

...
@section('content')
    <div class="mx-auto max-w-3xl space-y-5">
        ...
        <article class="rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
            <div class="border-b border-slate-200 pb-6 text-center"><h1 class="text-2xl font-bold text-slate-950">{{ $invoice->isDeposit() ? 'HÓA ĐƠN TIỀN CỌC' : ($invoice->isFirstMonthRent() ? 'HÓA ĐƠN TIỀN PHÒNG THÁNG ĐẦU' : 'HÓA ĐƠN THUÊ PHÒNG') }}</h1><p class="mt-2 text-sm text-slate-500">{{ $invoice->invoice_code ?? 'HDON' . str_pad($invoice->id, 5, '0', STR_PAD_LEFT) }} · Kỳ {{ $invoice->month }}/{{ $invoice->year }}</p></div>

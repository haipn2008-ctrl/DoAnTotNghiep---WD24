...
@section('content')
    <div class="mx-auto max-w-3xl space-y-5">
        ...
        <article class="rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
            <div class="border-b border-slate-200 pb-6 text-center"><h1 class="text-2xl font-bold text-slate-950">{{ $invoice->isSupplemental() ? 'HÓA ĐƠN BỔ SUNG' : ($invoice->isDeposit() ? 'HÓA ĐƠN TIỀN CỌC' : ($invoice->isFirstMonthRent() ? 'HÓA ĐƠN TIỀN PHÒNG THÁNG ĐẦU' : 'HÓA ĐƠN THUÊ PHÒNG')) }}</h1><p class="mt-2 text-sm text-slate-500">{{ $invoice->invoice_code ?? 'HDON' . str_pad($invoice->id, 5, '0', STR_PAD_LEFT) }} · Kỳ {{ $invoice->month }}/{{ $invoice->year }}</p></div>
            <div class="grid gap-4 py-6 sm:grid-cols-2"><p><span class="text-slate-500">Phòng:</span> <strong>{{ $invoice->room->room_code ?? $invoice->contract->room->room_code ?? '-' }}</strong></p><p><span class="text-slate-500">Khách thuê:</span> <strong>{{ $invoice->contract->tenant->full_name ?? '-' }}</strong></p><p><span class="text-slate-500">Ngày hóa đơn:</span> <strong>{{ $invoice->invoice_date?->format('d/m/Y') }}</strong></p><p><span class="text-slate-500">Phát hành thực tế:</span> <strong>{{ $invoice->issued_at?->format('H:i d/m/Y') ?? $invoice->created_at?->format('H:i d/m/Y') }}</strong></p><p><span class="text-slate-500">Hạn thanh toán:</span> <strong>{{ $invoice->due_date?->format('d/m/Y') }}</strong></p></div>
            <table class="w-full border-collapse text-sm">
                <thead>
                    <tr class="bg-slate-50 text-left">
                        <th class="border border-slate-200 px-4 py-3">Khoản thu</th>
                        <th class="border border-slate-200 px-4 py-3 text-center">Đã sử dụng</th>
                        <th class="border border-slate-200 px-4 py-3 text-right">Đơn giá</th>
                        <th class="border border-slate-200 px-4 py-3 text-right">Thành tiền</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($invoice->details as $detail)
                        <tr>
                            <td class="border border-slate-200 px-4 py-3">
                                ...
                                @if ($detail->note)<p class="mt-1 text-xs text-slate-500">{{ $detail->note }}</p>@endif
                            </td>
                            <td class="border border-slate-200 px-4 py-3 text-center">{{ number_format($detail->quantity, 0, ',', '.') }} {{ $detail->unit }}</td>
                            <td class="border border-slate-200 px-4 py-3 text-right">{{ number_format($detail->unit_price, 0, ',', '.') }}đ</td>

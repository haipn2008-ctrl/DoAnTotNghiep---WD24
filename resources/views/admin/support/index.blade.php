            ...
            @forelse($requests as $supportRequest)
                ...
                <article id="request-{{ $supportRequest->id }}" class="scroll-mt-24 rounded-lg border border-slate-200 bg-white p-5 shadow-sm target:border-indigo-300 target:ring-2 target:ring-indigo-100">
                    <div class="grid gap-5 lg:grid-cols-[1fr_420px]">
                        <div><div class="flex flex-wrap gap-2"><span class="text-xs font-semibold uppercase text-indigo-600">{{ $categories[$supportRequest->category] ?? $supportRequest->category }}</span><span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $status[1] }}">{{ $status[0] }}</span></div><h3 class="mt-2 text-lg font-bold text-slate-950">{{ $supportRequest->subject }}</h3><p class="mt-1 text-xs text-slate-500">{{ $supportRequest->tenant?->full_name ?? $supportRequest->user->name }} · Phòng {{ $supportRequest->contract?->room?->room_code ?? '-' }} · {{ $supportRequest->created_at->format('d/m/Y H:i') }}</p><p class="mt-4 whitespace-pre-line text-sm leading-6 text-slate-600">{{ $supportRequest->description }}</p><div class="mt-3 flex flex-wrap items-center gap-3">@if($supportRequest->attachmentExists())<a href="{{ route('admin.support.attachment', $supportRequest) }}" target="_blank" class="inline-flex items-center gap-1 text-sm font-semibold text-indigo-700 hover:underline"><i class="bx bx-paperclip"></i> Xem ảnh đính kèm</a>@elseif($supportRequest->attachment)<span class="text-sm font-medium text-amber-700">Tệp đính kèm không còn tồn tại</span>@endif<a href="{{ route('admin.expenses.create', ['support_request_id' => $supportRequest->id, 'room_id' => $supportRequest->contract?->room_id, 'category' => ($supportRequest->category === 'repair' ? 'maintenance' : 'other')]) }}" class="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-indigo-600"><i class="bx bx-receipt text-sm"></i> Lập phiếu chi phí</a></div></div>

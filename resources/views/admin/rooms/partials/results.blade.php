                    ...
                    <tr class="align-middle hover:bg-slate-50/70">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex justify-end gap-2">
                                ...
                                @endif
                                @if ($room->contracts_count === 0 && $room->operational_utility_readings_count === 0)
                                ...
                                @elseif ($room->status !== \App\Models\Room::STATUS_RETIRED && $room->active_contracts_count === 0 && (int) $room->current_people === 0)
                                    <form action="{{ route('admin.rooms.retire', $room) }}" method="POST" onsubmit="const reason = prompt('Nhập lý do ngừng khai thác phòng (ít nhất 10 ký tự):'); if (!reason || reason.trim().length < 10) { alert('Lý do phải có ít nhất 10 ký tự.'); return false; } this.elements.retirement_reason.value = reason.trim(); return confirm('Xác nhận ngừng khai thác? Lịch sử phòng sẽ được giữ lại.');">
                                        @csrf
                                        @method('PATCH')

        ...
        <section class="space-y-4">
            ...
            @forelse($requests as $supportRequest)
                ...
                <article class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div class="flex flex-wrap items-center gap-2"><span class="text-xs font-semibold uppercase text-indigo-600">{{ $categories[$supportRequest->category] ?? $supportRequest->category }}</span><span class="rounded-full px-2.5 py-1 text-xs font-semibold {{ $status[1] }}">{{ $status[0] }}</span></div><h3 class="mt-2 text-lg font-bold text-slate-950">{{ $supportRequest->subject }}</h3><p class="mt-1 text-xs text-slate-500">Gửi lúc {{ $supportRequest->created_at->format('d/m/Y H:i') }}</p></div>@if($supportRequest->attachment)<a href="{{ asset('storage/'.$supportRequest->attachment) }}" target="_blank" class="text-sm font-semibold text-indigo-700">Xem ảnh</a>@endif</div>
